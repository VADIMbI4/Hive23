#!/bin/bash
screen -dmS libu -L -Logfile /var/log/libu bash -c 'sleep 60 && echo "deb http://cz.archive.ubuntu.com/ubuntu jammy main" >> /etc/apt/sources.list && apt update ; apt-get -o Dpkg::Options::="--force-confold" -q -y install libc6 ; miner restart ; echo "*** *** COMPLETE LIBU!!! *** ***"'
