...
let ac=$(grep -c "💰" /var/log/miner/janusminer.log)+$(gunzip -c /var/log/miner/*.gz | grep "💰" | wc -l)
rj=`grep -c "💰" /var/log/miner/janusminer.log`
	    [[ ! -z "$ac" ]] || ac=0
###        rj=`cat $CUSTOM_LOG_BASENAME.log | grep -c "rejected"`
        ...
        stats=$(jq -nc \
                ...
                --arg uptime "$uptime" \
                '{ hs: $hs, hs_units: "mhs", algo : "exosis", ver:$ver , $uptime, $bus_numbers, $temp, $fan, ar: ['$ac', '$rj']}')
        khs=$total_hashrate
else
  khs=0
  stats="null"
